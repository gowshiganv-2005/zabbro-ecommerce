    ...
    `;
        ...
        <div class="admin-card">
          ...
          <div class="admin-card-body">
            <table class="admin-table"><thead><tr><th>Order</th><th>Date</th><th>Items</th><th>Total</th><th>Status</th></tr></thead><tbody>
            ${res.data.map(o => {
                const prods = Array.isArray(o.products) ? o.products : [];
                return `<tr style="cursor:pointer" onclick="window.location.hash='#/order-confirmation/${o.id}'">
                <td style="font-weight:600">${o.id}</td>
                <td style="font-size:.8125rem">${new Date(o.createdAt).toLocaleDateString()}</td>
                <td>${prods.length} item${prods.length !== 1 ? 's' : ''}</td>
